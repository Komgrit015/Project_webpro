...
<?php
	...
	if(!isset($ins)){
		...
		if(!$check){
			...
			echo '<tr>';
			echo '<form name="insert" method="post" action = "customer_insert.php">';
			echo '<td>';
			...
			echo '<td>';
			echo '<font color="red"> ';
		    echo "NO INFORMATION YOU CAN ADD CUSTOMER";
			echo '</font>';
			echo '</td>';
			echo '</tr>';
			echo '</table>';
			...
			while($row=mysqli_fetch_array($check)){
				...
				echo '<form name="up_'.$row[0].'"  method="post" action="customer_update.php">';
				...
				echo '<form name = "delete_'.$row[0].'" method = "post" action = "customer_delete.php">';
