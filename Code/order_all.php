...
<?php
	...
	if(!isset($ins)){
		...
		if(!$check){
			...
			echo '<tr>';
			echo '<form name="insert" method="post" action = "orders_insert.php">';
			echo '<td>';
			...
			echo '<td>';
			echo '<font color="red"> ';
		    echo "NO INFORMATION YOU CAN ADD ORDERS";
			echo '</font>';
			echo '</td>';
			echo '</tr>';
			echo '</table>';
			...
			while($row=mysqli_fetch_array($check)){
				...
				echo '<form name="up_'.$row[0].'"  method="post" action="orders_insert_detail.php">';
				echo '<input type = "hidden" name = "or_id" value = "'.$row[0].'">';
				echo '<td><input name = "up" type = "submit" value = "Detail"></td>';
			    echo '</form>';
				echo '<form name="confirm_'.$row[0].'"  method="post" action="orders_confirm.php">';
				echo '<input type = "hidden" name = "or_id" value = "'.$row[0].'">';
				echo '<input type = "hidden" name = "ship_name" value = "'.$row[3].'">';
				echo '<input type = "hidden" name = "or_date" value = "'.$row[5].'">';
				echo '<td><input name = "con" type = "submit" value = "Confirm"></td>';
			    echo '</form>';
				echo '<form name = "delete_'.$row[0].'" method = "post" action = "orders_delete.php">';
